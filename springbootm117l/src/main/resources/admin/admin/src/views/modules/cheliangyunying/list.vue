<template>
	<div class="main-content" :style='{"padding":"10px 30px 30px","background":"url(http://codegen.caihongy.cn/20230228/47752d6be3a94e8892a83ec2a70c8443.png)"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 30px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","borderRadius":"3px","background":"#fff","display":"block"}' >
					...
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">地点</label>
						<el-input v-model="searchForm.didian" placeholder="地点" clearable></el-input>
					</div>
				...
				<el-row :style='{"margin":"20px 0 20px 0","display":"flex"}'>
					...
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"rgba(255, 128, 0, 1)","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('cheliangyunying','车辆营业额')" type="warning" @click="chartDialog1()">车辆营业额</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"rgba(255, 128, 0, 1)","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('cheliangyunying','车辆营运里程')" type="warning" @click="chartDialog2()">车辆营运里程</el-button>
				</el-row>
			</el-form>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="yingyunshouruChart1" style="width:100%;height:600px;"></div>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="yingyunlichengChart2" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        ...
        var yingyunshouruChart1 = echarts.init(document.getElementById("yingyunshouruChart1"),'macarons');
        this.$http({
            url: `cheliangyunying/value/chepaihao/yingyunshouru`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].chepaihao);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].chepaihao
                    })
                }
                var option = {};
                option = {
                ...
                // 使用刚指定的配置项和数据显示图表。
                yingyunshouruChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    yingyunshouruChart1.resize();
                };
            }
        });
      })
...
//统计接口
    chartDialog2() {
      ...
      this.$nextTick(()=>{
        ...
        var yingyunlichengChart2 = echarts.init(document.getElementById("yingyunlichengChart2"),'macarons');
        this.$http({
            url: `cheliangyunying/value/chepaihao/yingyunlicheng`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                // 使用刚指定的配置项和数据显示图表。
                yingyunlichengChart2.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    yingyunlichengChart2.resize();
                };
            }
        });
      })
