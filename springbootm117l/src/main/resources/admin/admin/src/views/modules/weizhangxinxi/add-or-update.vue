	...
	<div class="addEdit-block" :style='{"padding":"10px 30px 30px","background":"url(http://codegen.caihongy.cn/20230228/47752d6be3a94e8892a83ec2a70c8443.png)"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="违章情况" prop="weizhangqingkuang">
					<el-input
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.weizhangqingkuang" label="违章情况" prop="weizhangqingkuang">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}'>{{ruleForm.weizhangqingkuang}}</span>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="违章处罚" prop="weizhangchufa">
					<el-input
					  style="min-width: 200px; max-width: 600px;"
					  type="textarea"
					  :rows="8"
					  placeholder="违章处罚"
					  v-model="ruleForm.weizhangchufa" >
					</el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.weizhangchufa" label="违章处罚" prop="weizhangchufa">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}'>{{ruleForm.weizhangchufa}}</span>
				</el-form-item>
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='weizhangchufa'){
							this.ruleForm.weizhangchufa = obj[o];
							this.ro.weizhangchufa = true;
							continue;
						}
				}
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `weizhangxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.weizhangxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
