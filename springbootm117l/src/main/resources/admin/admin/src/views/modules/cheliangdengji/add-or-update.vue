...
export default {
	...
	methods: {
		...
		init(id,type) {
			...
			gonghaoChange () {
				this.$http({
					url: `follow/jiashiyuan/gonghao?columnValue=`+ this.ruleForm.gonghao,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `cheliangdengji/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.cheliangdengjiCrossAddOrUpdateFlag = false;
						 this.parent.search();
