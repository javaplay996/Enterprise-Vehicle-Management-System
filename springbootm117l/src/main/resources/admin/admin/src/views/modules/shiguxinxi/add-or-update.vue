	...
	<div class="addEdit-block" :style='{"padding":"10px 30px 30px","background":"url(http://codegen.caihongy.cn/20230228/47752d6be3a94e8892a83ec2a70c8443.png)"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="车辆编号" prop="cheliangbianhao">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="事故处理" prop="shiguchuli">
					<el-input
					  style="min-width: 200px; max-width: 600px;"
					  type="textarea"
					  :rows="8"
					  placeholder="事故处理"
					  v-model="ruleForm.shiguchuli" >
					</el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.shiguchuli" label="事故处理" prop="shiguchuli">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}'>{{ruleForm.shiguchuli}}</span>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="事故处罚" prop="shiguchufa">
					<el-input
					  style="min-width: 200px; max-width: 600px;"
					  type="textarea"
					  :rows="8"
					  placeholder="事故处罚"
					  v-model="ruleForm.shiguchufa" >
					</el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.shiguchufa" label="事故处罚" prop="shiguchufa">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}'>{{ruleForm.shiguchufa}}</span>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-if="type!='info'"  label="事故详情" prop="shiguxiangqing">
					<editor 
						style="min-width: 200px; max-width: 600px;"
						v-model="ruleForm.shiguxiangqing" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.shiguxiangqing" label="事故详情" prop="shiguxiangqing">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.shiguxiangqing"></span>
                </el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='shiguchuli'){
							this.ruleForm.shiguchuli = obj[o];
							this.ro.shiguchuli = true;
							continue;
						}
						if(o=='shiguchufa'){
							this.ruleForm.shiguchufa = obj[o];
							this.ro.shiguchufa = true;
							continue;
						}
						if(o=='shiguxiangqing'){
							this.ruleForm.shiguxiangqing = obj[o];
							this.ro.shiguxiangqing = true;
							continue;
						}
						if(o=='dengjishijian'){
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.shiguxiangqing = this.ruleForm.shiguxiangqing.replace(reg,'../../../springbootm117l/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shiguxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shiguxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
